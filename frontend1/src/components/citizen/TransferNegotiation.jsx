...
const TransferNegotiation = () => {
  ...
  const downloadNegotiationSummary = () => {
    const latestReceipt = [...messages].reverse().find((msg) => ((msg.messageType || msg.message_type || '').toUpperCase()) === 'RECEIPT_PROPOSAL');
    const latestChallan = [...messages].reverse().find((msg) => ((msg.messageType || msg.message_type || '').toUpperCase()) === 'CHALLAN');
    const receiptPayload = latestReceipt ? parseMessagePayload(latestReceipt) : {};
    const challanPayload = latestChallan ? parseMessagePayload(latestChallan) : {};
    const challanKey = latestChallan ? getMessageKey(latestChallan) : '';
    const paidReceipt = (challanKey && paidChallans?.[challanKey]) || challanPayload.receipt || null;
    const paymentState = challanPayload.status === 'PAID' || paidReceipt || challanPayload.txnRef ? 'PAID' : (latestChallan ? 'CHALLAN ISSUED' : 'NOT GENERATED');
    const latestOfferValue = Number(channel.agreed_price || lastOffer || channel.transfer_amount || 0);

    const html = `<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8" />
  <title>Negotiation Summary - ${TRANSFER_ID || CHANNEL_ID}</title>
  <style>
    body { font-family: Arial, sans-serif; padding: 28px; color: #0f172a; background: #f8fafc; }
    .card { max-width: 880px; margin: 0 auto; background: #fff; border: 1px solid #dbe4ea; border-radius: 22px; overflow: hidden; }
    .head { background: linear-gradient(135deg, #095c5c, #0D7C7C); color: white; padding: 24px 28px; }
    ...
    .note { margin-top: 16px; background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 14px; padding: 14px; line-height: 1.65; color: #1e3a8a; }
  </style>
</head>
<body>
  <div class="card">
    <div class="head">
      ...
      <p>Channel ${CHANNEL_ID || 'N/A'} · Transfer ${TRANSFER_ID || 'N/A'}</p>
    </div>
    <div class="body">
      <div class="grid">
        <div class="box"><div class="label">Property ID</div><div class="value">${channel.property_id || 'N/A'}</div></div>
        <div class="box"><div class="label">Your Role</div><div class="value">${myRole || 'N/A'}</div></div>
        <div class="box"><div class="label">Listed Amount</div><div class="value">PKR ${Number(channel.transfer_amount || 0).toLocaleString('en-PK')}</div></div>
        <div class="box"><div class="label">Latest Confirmed Amount</div><div class="value">PKR ${latestOfferValue.toLocaleString('en-PK')}</div></div>
        <div class="box"><div class="label">Agreement Flags</div><div class="value">Seller: ${channel.seller_agreed ? 'YES' : 'NO'} · Buyer: ${channel.buyer_agreed ? 'YES' : 'NO'}</div></div>
        <div class="box"><div class="label">Payment State</div><div class="value">${paymentState}</div></div>
        <div class="box"><div class="label">Receipt Proposal</div><div class="value">${receiptPayload.price ? `PKR ${Number(receiptPayload.price).toLocaleString('en-PK')} for ${receiptPayload.durationDays || 0} day(s)` : 'No receipt proposal yet'}</div></div>
        <div class="box"><div class="label">Message Count</div><div class="value">${messages.length}</div></div>
      </div>
      <div class="note">
        ${challanPayload.challanId ? `Challan ID: ${challanPayload.challanId}<br />` : ''}
        ${challanPayload.txnRef || paidReceipt?.txnRef ? `TXN Ref: ${challanPayload.txnRef || paidReceipt?.txnRef}<br />` : ''}
        ${paidReceipt?.completedAt || challanPayload.paidAt ? `Paid At: ${new Date(paidReceipt?.completedAt || challanPayload.paidAt).toLocaleString('en-PK')}<br />` : ''}
        This summary is generated from the live negotiation room so buyer, seller, and support staff can review the current workflow without scrolling through the full chat.
      </div>
    </div>
  </div>
  <script>setTimeout(() => window.print(), 400);</script>
    ...
    if (!summaryWindow) {
      toast('Allow pop-ups to download the negotiation summary', 'error');
      return;
    }
    ...
    <CitizenLayout title={navTitle}>
      ...
      <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:16, overflow:'hidden', boxShadow:'0 1px 8px rgba(0,0,0,.06)', height:'calc(100vh - 124px)', display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ background:'#f8fafc', borderBottom:'1px solid #e5e7eb', padding:'7px 16px', display:'flex', alignItems:'center', gap:10, flexWrap:'wrap', flexShrink:0 }}>
          ...
          <button
            onClick={downloadNegotiationSummary}
            style={{
              padding:'6px 10px',
              borderRadius:10,
          ...
          >
            <i className="fas fa-file-export" style={{ fontSize:'.68rem' }} />
            Summary
          </button>
