    ...
    <div style={{
      ...
      <div>
        ...
        <div style={{ fontSize: ".85rem", color: T.text2, marginTop: 8 }}>
          {frozen
            ? listing.freeze_reason_label || "Marketplace locked by dispute hold"
            : coOwned
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", flexWrap: "wrap" }}>
        <button
          ...
          style={{
            ...
            borderRadius: 10,
            border: "none",
            background: saleBlocked ? "#94A3B8" : `linear-gradient(135deg,${T.primaryDark},${T.primary})`,
            color: "#fff",
            fontWeight: 700,
            fontSize: ".88rem",
        ...
        for (const item of (ownedData.properties || [])) {
          if (!listingMap.has(item.property_id)) {
            listingMap.set(item.property_id, {
              ...
              is_frozen: Boolean(item.is_frozen),
              has_co_owners: false,
              ownership_model: 'SOLE',
              active_co_owner_count: 0,
              co_owner_summary: null,
              freeze_reason_label: item.freeze_reason_label || null,
    ...
    <CitizenLayout title="Seller Portal">
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
          ...
          <div>
            <p style={{ margin: 0, fontSize: '.68rem', fontWeight: 800, color: T.muted, textTransform: 'uppercase', letterSpacing: '.14em' }}>
              Seller Portal
            </p>
            <h1 style={{ margin: '3px 0 0', fontSize: '1.45rem', fontWeight: 800, color: T.text, lineHeight: 1.1, letterSpacing: '-.02em', fontFamily: "'Plus Jakarta Sans','DM Sans',system-ui,sans-serif" }}>
              Seller Portal
            </h1>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12 }}>
          {[
            { label: 'Listed',           value: stats.listed,    icon: 'fas fa-store',        border: '#C8E0E0', iconBg: '#E6F4F2', iconColor: '#0D7C7C' },
            { label: 'Pending Requests', value: stats.pending,   icon: 'fas fa-inbox',        border: '#FDE68A', iconBg: '#FFFBEB', iconColor: '#D97706' },
            { label: 'Accepted',         value: stats.accepted,  icon: 'fas fa-check-circle', border: '#BBF7D0', iconBg: '#ECFDF5', iconColor: '#059669' },
            { label: 'Ready for Chat',   value: stats.readyChat, icon: 'fas fa-comments',     border: '#C8E0E0', iconBg: '#E6F4F2', iconColor: '#0D7C7C' },
          ].map((stat) => (
        ...
        <section style={{
          ...
          {loading ? (
            ...
            <div style={{
              ...
              No approved are available for sale.
            </div>
          ) : (
            <>
              {/* Table header */}
