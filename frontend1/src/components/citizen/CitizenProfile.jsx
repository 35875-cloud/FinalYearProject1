...
const CitizenProfile = () => {
  ...
  const updateGender = async (gender) => {
    ...
    try {
      ...
      const d = await r.json();
      if (!d.success || !d.user) throw new Error(d.message || 'Could not update profile gender');
      setUser(d.user);
    } catch (e) {
      setError(e.message);
    } finally {
      ...
      {!loading && user && (
        <div style={{ maxWidth: 680 }}>
          ...
          {!user.gender && (
            <div style={{
              ...
              <div style={{ fontWeight: 700, color: '#9a3412', marginBottom: 6 }}>
                Profile gender is missing
              </div>
              <div style={{ fontSize: 13, color: '#9a3412', lineHeight: 1.6, marginBottom: 12 }}>
                Succession needs stored profile gender to calculate husband and wife shares correctly. Save it once here.
              </div>
          ...
          <Card icon="fas fa-id-card" title="Personal Information">
            ...
            <InfoRow icon="fas fa-venus-mars" label="Gender"      value={user.gender || 'Not set'} />
