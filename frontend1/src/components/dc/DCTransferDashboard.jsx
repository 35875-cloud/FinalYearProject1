...
const downloadTransferReviewSheet = (record) => {
  if (!record) return;

  const voteRows = (record.votes || []).map((vote) => `
    <tr>
      <td>${vote.lro_name || vote.lro_node_id || 'LRO Node'}</td>
  ...
  <html>
    <head>
      <meta charset="utf-8" />
      <title>Transfer Review - ${record.transfer_id}</title>
      <style>
    ...
    <body>
      <div class="tag">DC Transfer Review Sheet</div>
      <h1>${record.transfer_id}</h1>
      <p>Prepared for Deputy Commissioner review from the transfer voting queue.</p>
      <table>
        <tbody>
          <tr><th>Property ID</th><td>${record.property_id || 'N/A'}</td></tr>
          <tr><th>Seller</th><td>${record.seller_name || 'N/A'}</td></tr>
          <tr><th>Buyer</th><td>${record.buyer_name || 'N/A'}</td></tr>
          <tr><th>District / Tehsil</th><td>${[record.district, record.tehsil].filter(Boolean).join(', ') || 'N/A'}</td></tr>
          <tr><th>Channel ID</th><td>${record.channel_id || 'N/A'}</td></tr>
          <tr><th>Agreed Amount</th><td>PKR ${Number(record.displayAmount || 0).toLocaleString('en-PK')}</td></tr>
  ...
  link.href = url;
  link.download = `Transfer-Review-${record.transfer_id || Date.now()}.html`;
...
const DCTransferDashboard = () => {
  ...
  return (
    <DCLayout title="Final Transfer Approval">
      <div style={{ display: 'grid', gap: '1.35rem' }}>
        ...
        <Panel
          ...
          subtitle="This shared final-decision feed keeps both registration and transfer approvals visible from the transfer side too."
          action={
              ...
              {!selectedRecord ? (
                ...
                <div style={{ display: 'grid', gap: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '.8rem', flexWrap: 'wrap' }}>
                    {/* <ActionButton onClick={() => downloadTransferReviewSheet(selectedRecord)} outline>
