...
const downloadRegistrationCaseSheet = (record) => {
  if (!record) return;
...
<head>
  <meta charset="UTF-8" />
  <title>Registration Case Sheet - ${record.property_id}</title>
  <style>
    ...
    .section { margin-top: 16px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 14px; padding: 14px; line-height: 1.65; }
  </style>
</head>
<body>
  <div class="card">
    <div class="head">
      <h1>Registration Case Sheet</h1>
      <p>5-node LRO voting review snapshot</p>
    </div>
    <div class="body">
      <div class="grid">
        <div class="box"><div class="label">Property ID</div><div class="value">${record.property_id}</div></div>
        <div class="box"><div class="label">Owner</div><div class="value">${record.owner_name || 'N/A'}<br/>${record.owner_cnic || 'N/A'}</div></div>
        <div class="box"><div class="label">Location</div><div class="value">${[record.district, record.tehsil, record.mauza].filter(Boolean).join(', ') || 'N/A'}</div></div>
        <div class="box"><div class="label">Progress</div><div class="value">${record.approvals}/${record.threshold} approvals · Reject ${record.rejections}</div></div>
      </div>
      <div class="section">
        <strong>Vote History</strong><br/>
        ${(record.votes?.length
          ? record.votes.map((vote) => `${vote.lro_name || vote.lro_node_id}: ${vote.vote} (${fmtDateTime(vote.voted_at)})${vote.reason ? ` - ${vote.reason}` : ''}`).join('<br/>')
          : 'No vote history yet.')}
      </div>
    </div>
  </div>
...
const LROVotingPanel = () => {
  ...
  const loadQueue = async () => {
    ...
    setError('');
    try {
      const response = await fetch(`${base}/api/registration-voting/lro/queue`, {
        headers: { Authorization: `Bearer ${authToken}` },
      });
      const data = await response.json();
      if (!response.ok || !data.success) {
        throw new Error(data.message || 'Unable to load registration voting queue');
      }
  ...
  useEffect(() => {
    ...
    if (!records.some((item) => item.property_id === selectedId)) {
      setSelectedId(records[0].property_id);
    }
  }, [records, selectedId]);

  const selectedRecord = useMemo(
    () => records.find((item) => item.property_id === selectedId) || null,
    [records, selectedId]
  );
  ...
  const castVote = async (vote) => {
    ...
    setMessage(null);
    try {
      const response = await fetch(`${base}/api/registration-voting/lro/${selectedRecord.property_id}/vote`, {
        method: 'POST',
        headers: {
      ...
      setDecisionNote('');
      setMessage({ type: 'success', text: `Your ${vote.toLowerCase()} vote was recorded for ${selectedRecord.property_id}.` });
      await loadQueue();
    } catch (err) {
  ...
  return (
    <OfficerLayout title="Registration Voting">
      <div style={{ display: 'grid', gap: '1.35rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
            <div>
              <div style={{ fontFamily: "'Sora', sans-serif", fontWeight: 800, fontSize: '1.55rem', color: T.text, marginBottom: 6 }}>
                5-Node Registration Voting
              </div>
              <div style={{ color: T.text2, fontSize: '.92rem', maxWidth: 760 }}>
                Cast one vote per case from your mapped LRO node. Once a case reaches three approvals, it moves to the DC queue for final approval and permanent registration.
              </div>
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: '1rem' }}>
          ...
          <SummaryCard label="Finalized Cases" value={summary.finalized} icon="fas fa-circle-check" tone={T.success} />
        </div>
        ...
        {loading ? (
          <Panel title="Loading queue">
            <div style={{ textAlign: 'center', color: T.muted, padding: '3.5rem 0' }}>
              ...
              Loading registration vote queue...
            </div>
          </Panel>
        ) : !records.length ? (
          <Panel title="Registration Voting Queue" subtitle="No registration cases are in the shared vote queue right now.">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(430px, 1.02fr) minmax(360px, .98fr)', gap: '1.25rem', alignItems: 'start' }}>
            <Panel title="Case List" subtitle="Open a row to inspect the case and cast your single node vote.">
              <div style={{ border: `1px solid ${T.border}`, borderRadius: 18, overflow: 'hidden' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '56px minmax(0,1.55fr) 156px 132px', background: '#f8fafc', color: T.text2, fontWeight: 800, fontSize: '.76rem', letterSpacing: .35, textTransform: 'uppercase', padding: '.8rem 1rem', gap: '.8rem' }}>
                  <div>#</div>
                  <div>Property</div>
                  <div>Stage</div>
                  <div>Votes</div>
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          display: 'grid',
                          gridTemplateColumns: '56px minmax(0,1.55fr) 156px 132px',
                          gap: '.8rem',
                          padding: '.95rem 1rem',
                          textAlign: 'left',
                          cursor: 'pointer',
            ...
            <Panel title={selectedRecord ? selectedRecord.property_id : 'Case Details'} subtitle={selectedRecord ? 'Inspect the case, its vote trail and your current action state.' : 'Select a case first.'}>
              {!selectedRecord ? (
                ...
                <div style={{ display: 'grid', gap: '1rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '.75rem', flexWrap: 'wrap' }}>
                      {/* <button
                        onClick={() => downloadRegistrationCaseSheet(selectedRecord)}
                        style={{
                          border: 'none',
                          borderRadius: 12,
                          padding: '10px 14px',
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(170px,1fr))', gap: '.85rem' }}>
                    ...
                    <DetailItem label="Submitted By Node" value={selectedRecord.submitted_by_node} />
                  ...
                  <div style={{ borderRadius: 16, border: '1px solid #bfdbfe', background: '#eff6ff', padding: '1rem 1.05rem', color: '#1d4ed8' }}>
                    ...
                    <div style={{ fontSize: '.84rem', lineHeight: 1.6 }}>
                      {String(selectedRecord.status).toUpperCase() === 'VOTING' && 'This case is waiting for node votes. Each mapped LRO node can vote once.'}
                      {String(selectedRecord.status).toUpperCase() === 'READY_FOR_DC' && 'This case has crossed the approval threshold and is now waiting for DC final review.'}
                      {String(selectedRecord.status).toUpperCase() === 'FINALIZED' && 'This case has completed the registration flow and is now approved.'}
                      {String(selectedRecord.status).toUpperCase() === 'REJECTED' && 'This case has been rejected and is no longer in the active approval path.'}
                    </div>
                  </div>
                  ...
                  <div style={{ background: '#f8fafc', border: `1px solid ${T.border}`, borderRadius: 18, padding: '1rem' }}>
                    ...
                    {selectedRecord.currentUserVote ? (
                      ...
                      <div style={{ color: T.text2, fontSize: '.84rem' }}>
                        Voting is not open for your node on this case right now.
                      </div>
                    ) : (
                  ...
                  <div style={{ display: 'grid', gap: '.75rem' }}>
                    ...
                    {!selectedRecord.votes?.length ? (
                      <div style={{ color: T.muted, fontSize: '.84rem' }}>No votes have been recorded on this case yet.</div>
