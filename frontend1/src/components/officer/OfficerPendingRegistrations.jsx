...
const downloadIntakeSheet = (record) => {
  if (!record) return;
  ...
  <html>
    <head>
      <meta charset="utf-8" />
      <title>Registration Intake - ${record.property_id}</title>
      <style>
        body { font-family: Arial, sans-serif; margin: 28px; color: #102033; }
        h1 { margin: 0 0 8px; font-size: 24px; }
        p { margin: 0 0 18px; color: #4b5563; }
        .tag { display:inline-block; padding:6px 10px; border-radius:999px; background:#fff7ed; color:#c2410c; font-weight:700; font-size:12px; margin-bottom:16px; }
    ...
    <body>
      <div class="tag">LRO Registration Intake Sheet</div>
      <h1>${record.property_id}</h1>
      <p>Prepared from the pending registration intake queue before the case enters node voting.</p>
      <table>
        <tbody>
          ...
          <tr><th>Khewat / Khatooni</th><td>${record.khewat_no || 'N/A'} / ${record.khatooni_no || 'N/A'}</td></tr>
          <tr><th>Khasra</th><td>${record.khasra_no || 'N/A'}</td></tr>
          ...
          <tr><th>Address</th><td>${record.address || 'N/A'}</td></tr>
          <tr><th>Created At</th><td>${fmtDateTime(record.created_at)}</td></tr>
        </tbody>
      </table>
    </body>
  </html>`;
  ...
  link.href = url;
  link.download = `Registration-Intake-${record.property_id || Date.now()}.html`;
              ...
              {!selectedRecord ? (
                ...
                <div style={{ display: 'grid', gap: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '.8rem', flexWrap: 'wrap' }}>
                    <ActionButton onClick={() => downloadIntakeSheet(selectedRecord)} outline>
