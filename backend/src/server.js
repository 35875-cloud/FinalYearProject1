...
// Changes: removed gun_relay, added /api/p2p routes
// =====================================================
...
app.use(cors({ origin: "*", methods: ["GET","POST","PUT","DELETE","PATCH"], credentials: true, allowedHeaders: ["Content-Type","Authorization"] }));
app.use(express.json({ limit: "10mb" }));
app.use(express.urlencoded({ extended: true, limit: "10mb" }));
if (process.env.NODE_ENV !== "production") app.use(morgan("dev"));

// ── Routes ────────────────────────────────────────────────────────
app.get("/", (req, res) => res.json({ message: "✅ Backend Running", websocket: "enabled", e2e: "enabled" }));
...
process.on("SIGTERM", () => { httpServer.close(() => { pool.end(); process.exit(0); }); });
process.on("SIGINT",  () => { httpServer.close(() => { pool.end(); process.exit(0); }); });
